...
[NOTE]
====
An admin user works well in this workshop setting. For actual deployment, use a formal identity provider to access the cluster and grant the user admin privileges.
====
...
. You can now use the cluster as an admin user, which will suffice for this workshop. For actual deployment, it is highly recommended to set up an identity provider, which is explained in the next tutorial.